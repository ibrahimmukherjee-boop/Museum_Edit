    ...
    <div className="journey-ambient min-h-screen">
      <div className="mx-auto max-w-lg px-4 pb-16 pt-6">
        ...
        <GlassPanel variant="dark" className="mt-8 overflow-hidden p-8 text-center">
          ...
          <h1 className="mt-2 font-[Cinzel] text-3xl text-amber-50">Speak with Leonardo</h1>
          <div className="mx-auto mt-6 max-w-md overflow-hidden rounded border border-white/15 shadow-lg">
            <img
              src={`${import.meta.env.BASE_URL}art/annunciation.png`}
              alt="The Annunciation"
              className="block w-full object-cover"
              onError={(e) => {
                e.currentTarget.src = `${import.meta.env.BASE_URL}art/annunciation.jpg`;
              }}
            />
          </div>
          ...
          <div className="mt-8 flex flex-col gap-3">
            <Link
              to="/parlor"
              className="block rounded-md border-2 border-amber-50 bg-stone-950 px-6 py-3.5 font-[Cinzel] text-sm font-semibold tracking-wide text-amber-50 shadow-[0_4px_24px_rgba(0,0,0,0.45)] no-underline ring-2 ring-amber-400/50 hover:bg-stone-900 hover:text-white"
            >
              Enter the Parlor
            </Link>
            <Link
              to="/atelier"
              className="block rounded border border-amber-200/30 bg-white/8 px-6 py-3.5 font-[Cinzel] text-sm tracking-wide text-amber-50 no-underline hover:bg-white/14"
            >
              Enter the Atelier
            </Link>
          </div>
