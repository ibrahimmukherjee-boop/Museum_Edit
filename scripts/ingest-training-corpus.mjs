...
function chunkText(text, source, domain = "general", maxChunks = 48) {
  ...
  for (const s of sentences) {
    if (buf.length + s.length > 480) {
      if (buf.length > 60 && isSafeChunk(source, buf.trim())) chunks.push({ source, domain, text: buf.trim() });
      buf = s;
    } else {
      buf = buf ? `${buf} ${s}` : s;
    }
  ...
  if (buf.length > 60 && chunks.length < maxChunks && isSafeChunk(source, buf.trim())) {
    chunks.push({ source, domain, text: buf.trim() });
  }
  return chunks;
}
...
const THIRD_PERSON = /\b(The actual Leonardo|Leonardo's|Leonardo is|Leonardo was|He served Cesare|His humour)\b/i;

function isSafeChunk(source, text) {
  if (/polymath festival|transcription for|speaker_\d+/i.test(source)) return false;
  if (/SPEAKER_\d+|Transcription for|Polymath Festival:\s*transcription/i.test(text)) return false;
  if (source.startsWith("Personality:")) {
...
const unique = entries.filter((e, i, a) => a.findIndex((x) => x.text === e.text) === i).filter((e) => isSafeChunk(e.source, e.text));
